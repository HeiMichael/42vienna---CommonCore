...
/*   Updated: 2024/10/28 14:40:01 by miheider         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void BitcoinExchange::find_data(const std::string& date, double amount) {
    ...
    it2 = _data_base.lower_bound(date);  // Use lower_bound to find the closest date

    // Check if the date found is valid
    if (it2 == _data_base.end() || it2->first > date) {
        if (it2 != _data_base.begin()) {
            --it2;  // Move to the previous element if the current one is greater
        } else {
            std::cout << "No suitable date found in _data_base for " << date << std::endl;
            return;  // No suitable date found, exit early
        }
    }

    // At this point, it2 points to the closest earlier date

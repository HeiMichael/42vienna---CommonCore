...
/*   Updated: 2024/12/04 11:32:34 by miheider         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void PmergeMe::sort_back_vector() {
    ...
    for (i = 1; i < _helper_vector.size(); ++i) {
        ...
        std::vector<std::vector<int> >::iterator pos =
            std::lower_bound(temp.begin(), temp.begin() + (jt_idx + counter), second_half);
        temp.insert(pos, second_half);
        counter++;
    }
    if (!protrusion.empty()) {
        std::vector<std::vector<int> >::iterator pos =
            std::lower_bound(temp.begin(), temp.end(), protrusion);
        temp.insert(pos, protrusion);
    }
    _size_vector.erase(_size_vector.begin());
    _size_vector.erase(_size_vector.begin());
    _helper_vector = temp;
    if (_size_vector.size() >= 2 && _size_vector[1] >= 1) {
        sort_back_vector();
    }
    if (_struggler_vector >= 0) {
        ...
        std::vector<std::vector<int> >::iterator pos =
            std::lower_bound(_helper_vector.begin(), _helper_vector.end(), second_half);
...
PmergeMe::PmergeMe(int argc, char** argv): _struggler_deque(-1), _struggler_vector(-1) {
	double	elapsed_time;
	std::cout << "Before:\t";
	...
	if (argc == 3 || argc == 4) {
		...
		if (_input_deque[0] > _input_deque[1]) {
			first.push_front(_input_deque[1]);
            second.push_back(_input_deque[0]);
        } else {
            first.push_front(_input_deque[0]);
            second.push_back(_input_deque[1]);
        }
        comparisons++;
        _helper_deque.push_back(first);
        _helper_deque.push_back(second);
        if (argc == 4) {
            std::deque<int> third;
            third.push_back(_input_deque[2]);
            std::deque<std::deque<int> >::iterator pos = std::lower_bound(_helper_deque.begin(), _helper_deque.end(), third, compareWithCounter_deque);
            _helper_deque.insert(pos, third);
        }
	} else {	
        _size_deque.push_front(1);
        pair_deque();
    ...
    if (argc == 3 || argc == 4) {
		...
		if (_input_vector[0] > _input_vector[1]) {
			first.insert(first.begin(), _input_vector[1]);
            second.push_back(_input_vector[0]);
        } else {
            first.insert(first.begin(), _input_vector[0]);
            second.push_back(_input_vector[1]);
        }
        _helper_vector.push_back(first);
        _helper_vector.push_back(second);
        if (argc == 4) {
            std::vector<int> third;
            third.push_back(_input_deque[2]);
            std::vector<std::vector<int> >::iterator pos = std::lower_bound(_helper_vector.begin(), _helper_vector.end(), third);
            _helper_vector.insert(pos, third);
        }
	} else {	
        _size_vector.insert(_size_vector.begin(), 1);
        pair_vector();
    ...
    std::cout << "After: ";
    for (size_t i = 0; i <= _helper_vector.size() - 1; i++) {

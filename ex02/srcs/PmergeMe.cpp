...
/*   Updated: 2024/11/22 10:45:31 by miheider         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void PmergeMe::sort_back_deque() {
    ...
    // second_half.clear();
    // second_half.insert(second_half.end(), _helper_deque[0].begin() + _size_deque[0], _helper_deque[0].end());
    ...
    for (size_t i = 0; i < _helper_deque.size(); i++) {
		...
		if (jt_idx > static_cast<size_t>(_helper_deque.size()))
			continue ;
        if (_helper_deque[jt_idx].size() >= static_cast<size_t>(2 * _size_deque[0])) {
            second_half.insert(second_half.end(), _helper_deque[jt_idx].begin() + _size_deque[0], _helper_deque[jt_idx].end());
